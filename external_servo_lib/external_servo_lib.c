...
uint32_t pwm_set_freq_duty(uint32_t slice_num, uint32_t chan,
                                     uint32_t f, int d)
{
...
uint32_t pwm_get_wrap(uint32_t slice_num)
{
    // valid_params_if(PWM, slice_num >= 0 && slice_num < NUM_PWM_SLICES);
    return pwm_hw->slice[slice_num].top;
}

void pwm_set_dutyH(uint32_t slice_num, uint32_t chan, int32_t d)
{
